...
const Hero = () => {
  ...
  return <div className="h-screen relative flex flex-col items-center" style={{
    ...
    <div className="relative w-full max-w-[1490px] flex items-center justify-between pt-10 mx-auto px-10">
      ...
      <img src={cartIcon} alt="cartIcon" className='hidden md:block cursor-pointer'/>
      <HiMenuAlt3 size={30} className="block md:hidden cursor-pointer text-white" 
      onClick={()=> setShowMobileMenu((prev) => !prev)}
      />
      <div className={`block md:hidden w-full fixed ${!showMobileMenu ? "-top-[410px]" : "top-0"} left-0 bg-[#dde0e6] h-[410px] transition-all duration-[800ms] shadow-xl z-10 py-8 px-12 rounded-b-xl`}>
        ...
        <NavLink to="services" mobileMenu>Servicios</NavLink>
        <NavLink to='products' mobileMenu>Productos</NavLink>
        <NavLink to='reference' mobileMenu>Referencia</NavLink>
        <NavLink to='care' mobileMenu>Cuidado</NavLink>
        </ul>
    ...
    <FadeIn delay={0.2} direction="up" padding fullWidth>
      <div className="relative w-full xs:w-[460px] mt-11">
        <input type="text" placeholder="Buscar" className="rounded-full w-full pl-6 pr-[68px] py-4 bg-primary outline-none text-white text-base xs:text-lg placeholder-white" />
